...
export function Header() {
  ...
  return (
    <header
      ref={menuRef}
      className="sticky top-0 z-10 border-b border-[var(--card-border)] bg-[var(--header-bg)] backdrop-blur-md"
    >
      <div className="mx-auto flex max-w-[1120px] items-center gap-3 px-6 py-3.5">
        ...
        <button
          ...
          className="ml-auto grid h-9 w-9 place-items-center rounded-xl border border-[var(--card-border)] bg-[var(--card)] text-[var(--text)] transition-colors hover:bg-[var(--surface-hover)] lg:hidden"
        >
          <svg
            width="18"
            height="18"
      ...
      {menuOpen && (
        <div className="border-t border-[var(--card-border)] bg-[var(--header-bg)] lg:hidden">
          <div className="mx-auto flex max-w-[1120px] flex-col gap-1 px-6 py-3">
            <nav className="flex flex-col">
              {NAV_LINKS.map((l) => (
                <Link
                  ...
                  href={l.href}
                  onClick={closeMenu}
                  className="rounded-lg px-2 py-2 text-[15px] text-[var(--text)] no-underline transition-colors hover:bg-[var(--surface-hover)]"
                >
                  {l.label}
                </Link>
              ))}
            ...
            <div className="mt-2 flex flex-col gap-2 border-t border-[var(--card-border)] pt-3">
              <Link
                href="/upload"
                onClick={closeMenu}
                className="btn-brand inline-flex items-center justify-center gap-1.5 rounded-xl px-4 py-2.5 text-[14px] font-semibold"
              >
                Разобрать документ
              </Link>
              {authed ? (
                <>
                  <Link
                    href="/documents"
                    onClick={closeMenu}
                    className="inline-flex items-center justify-center rounded-xl border border-[var(--card-border)] bg-[var(--card)] px-4 py-2.5 text-[14px] font-semibold text-[var(--text)] no-underline transition-colors hover:bg-[var(--surface-hover)]"
                  >
                    Мои документы
                  </Link>
                  <Link
                  ...
                  <button
                    ...
                    className="inline-flex items-center justify-center rounded-xl border border-[var(--card-border)] bg-[var(--card)] px-4 py-2.5 text-[14px] font-semibold text-[var(--text)] transition-colors hover:bg-[var(--surface-hover)]"
                  >
                    Выйти
                  </button>
                </>
              ...
              <div className="flex items-center justify-between rounded-xl border border-[var(--card-border)] bg-[var(--card)] px-4 py-2.5">
                <span className="text-[14px] text-[var(--muted)]">Тема оформления</span>
                <ThemeToggle />
              </div>
            </div>
          </div>
