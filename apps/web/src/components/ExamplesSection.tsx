...
const EXAMPLES: Example[] = [
  {
    ...
    tabIcon: "📑",
    tabLabel: "Договор",
    fileName: "polis_strahovaniya_zhizni.pdf",
    severity: "urgent",
    severityText: "Срочно — у вас есть 14 дней, чтобы передумать и вернуть деньги",
    whatShort:
      "Договор добровольного страхования жизни и здоровья, оформленный вместе с ипотекой. Менеджер банка часто говорит «это обязательно», но по закону — нет. Действует «период охлаждения» 14 дней: можно отказаться и вернуть всю сумму. Внутри также прячутся условия о ставке по кредиту, привязка к договору и декларация о здоровье — нарушение любого пункта может лишить выплаты.",
    sender: "СК «Пример-Жизнь»",
    docType: "Полис добровольного страхования жизни и здоровья (к ипотеке)",
    dates: [
      { label: "Дата подписания", value: "10.05.2026" },
      { label: "Последний день, чтобы отказаться без потерь", value: "24.05.2026" },
      { label: "Срок договора", value: "10 лет (до 10.05.2036)" },
    ],
    amounts: [
      { label: "Взнос за первый год", value: "38 400 ₽" },
      { label: "Всего за 10 лет (по графику)", value: "≈ 412 000 ₽" },
      { label: "Скидка к ставке по ипотеке", value: "−1 % годовых" },
    ],
    summary:
      "Это не обязательная страховка, а добровольный договор на 10 лет почти на полмиллиона рублей. Если он вам не нужен — у вас есть 14 дней, чтобы отказаться и вернуть деньги. Скидка к ставке по ипотеке при этом тоже отвалится — посчитайте, что выгоднее.",
    goal: "Страховая хочет, чтобы вы платили взносы 10 лет подряд. Банк — чтобы вы держали страховку и сохраняли пониженную ставку.",
    steps: [
      "Сравните: 412 000 ₽ за 10 лет страховки vs прибавка к ипотеке +1 % годовых — посчитайте на калькуляторе банка, что дороже",
      "Если страховка не нужна — до 24 мая подайте заявление об отказе (бумажное, в офис страховой или почтой с уведомлением)",
      "Уточните в банке, сохранится ли пониженная ставка без страховки — иногда да, иногда нет",
      "Если оставляете — внимательно прочитайте декларацию о здоровье: при онкологии, ИБС или инвалидности в выплате могут отказать",
    ],
  },
  {
    id: "bank",
    tabIcon: "🏦",
    tabLabel: "Банк / коллекторы",
    fileName: "trebovanie_dosrochnogo_pogashniya.pdf",
    severity: "urgent",
    severityText: "Срочно — 30 дней до суда, но почти всё можно решить звонком",
    whatShort:
      "Банк прислал письмо с требованием досрочно погасить весь остаток кредита из-за двух пропущенных платежей. Звучит страшно, но это стандартный шаг перед судом: в 9 из 10 случаев решается реструктуризацией или кредитными каникулами — без потери жилья и без коллекторов.",
    sender: "ПАО «Пример-Банк», отдел взыскания",
    docType: "Требование о досрочном возврате кредита",
    dates: [
      { label: "Дата требования", value: "05.05.2026" },
      { label: "Срок добровольного погашения", value: "до 04.06.2026 (30 дней)" },
    ],
    amounts: [
      { label: "Остаток основного долга", value: "847 200 ₽" },
      { label: "Просрочка (2 платежа)", value: "62 800 ₽" },
      { label: "Пени и штрафы", value: "4 350 ₽" },
    ],
    summary:
      "Если внести просрочку 62 800 ₽ в ближайшие дни — банк, скорее всего, отзовёт требование. Если денег нет — позвоните в банк и попросите кредитные каникулы (по закону их обязаны рассмотреть). Главное — не молчать.",
    goal: "Банк хочет вернуть всю сумму или, что вероятнее, заставить вас выйти на связь и согласовать график.",
    steps: [
      "Позвоните в банк в первый же день — попросите реструктуризацию или кредитные каникулы (право по 106-ФЗ при падении дохода)",
      "Если есть деньги на просрочку 62 800 ₽ — внесите их, требование почти всегда отзывают",
      "Не берите новый кредит, чтобы закрыть этот — это путь в долговую яму",
      "Если позвонят коллекторы — записывайте разговор, угрозы и звонки ночью незаконны (230-ФЗ)",
    ],
  },
  {
    id: "zhkh",
    tabIcon: "🏠",
    tabLabel: "ЖКХ",
    fileName: "pereraschet_goryachey_vody.pdf",
    severity: "neutral",
    severityText: "Нейтрально — управляющая компания может ошибаться, проверьте сами",
    whatShort:
      "Управляющая компания прислала перерасчёт за горячую воду: говорит, что счётчик якобы не передавал показания и насчитала по нормативу — в 3 раза больше обычного. Это типовая ситуация, и в большинстве случаев перерасчёт можно отменить, если показания вы передавали.",
    sender: "ООО УК «Пример-Сервис»",
    docType: "Уведомление о перерасчёте платы за горячее водоснабжение",
    dates: [
      { label: "Период перерасчёта", value: "январь — март 2026" },
      { label: "Срок оплаты", value: "до 25.05.2026" },
    ],
    amounts: [
      { label: "Обычный платёж в месяц", value: "≈ 850 ₽" },
      { label: "Доначисление", value: "+ 6 200 ₽" },
    ],
    summary:
      "Это не суд и не долг с приставами — обычное письмо от УК. Если показания вы передавали (есть скриншоты или СМС от УК) — перерасчёт обязаны отменить по вашему заявлению. Если нет — оплатите по факту последних показаний.",
    goal: "УК хочет, чтобы вы доплатили 6 200 ₽ или сами доказали, что показания передавались.",
    steps: [
      "Найдите подтверждения передачи показаний: СМС, скриншоты из приложения, чеки оплаты с показаниями",
      "Напишите заявление в УК с просьбой пересчитать по фактическим показаниям, приложите доказательства",
      "Если УК откажет — жалоба в Госжилинспекцию региона через Госуслуги (бесплатно, рассматривают 30 дней)",
      "Параллельно проверьте, опломбирован ли счётчик и не истёк ли срок поверки — это влияет на расчёт",
    ],
  },
  {
    id: "phishing",
    ...
    <section id="examples" className="py-20">
      <div className="mx-auto max-w-[1120px] px-6">
        ...
        <h2 className="mb-3 text-[clamp(28px,3.5vw,40px)] font-extrabold tracking-[-0.01em]">
          Где сервис реально полезен
        </h2>
        <p className="mb-3 max-w-[640px] text-[17px] text-[var(--muted)]">
          В договорах со страховой, банком или коллекторами легко пропустить ловушку. С госорганами
          обычно проще — там и по телефону горячей линии всё спокойно объяснят. Выберите пример
          ближе к вашему случаю.
        </p>
        <p className="mb-8 max-w-[640px] rounded-md border border-red-500/30 bg-red-500/10 px-4 py-2 text-[13px] text-[var(--text)]">
          <b className="text-red-400">Важно:</b> договоры с банком, страховой или работодателем —
          это коммерческая тайна. Загружать чужие договоры в любые внешние сервисы (наш —
          не исключение) нельзя: это нарушение договора и личная ответственность.
          Разбирайте только свои личные документы.
        </p>
