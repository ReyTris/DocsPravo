    ...
    mainEntity: [
      {
        ...
        a: "Перед отправкой в ИИ имя, ИНН, паспорт, СНИЛС и УИН мы заменяем на простые пометки — вроде [имя_1], [инн_1]. Нейросеть видит только обезличенный текст и ничего не запоминает.",
      },
      {
    ...
    step: [
      {
        ...
        position: 1,
        name: "Загрузите",
        text: "Сфотографируйте письмо на телефон или прикрепите PDF из почты. Можно несколько страниц сразу.",
      },
      {
        "@type": "HowToStep",
        position: 2,
        name: "Скрываем личное",
        text: "Имя, ИНН, паспорт и адрес заменяем на пометки. В ИИ уходит обезличенный текст — ваши данные не утекают.",
      },
      {
        "@type": "HowToStep",
        position: 3,
        name: "Читаем за вас",
        text: "Определяем, кто прислал, что хочет, какие суммы и сроки указаны и чем грозит просрочка.",
      },
      {
        "@type": "HowToStep",
        position: 4,
        name: "Понятный ответ",
        text: "Что это, что делать сегодня и сколько у вас времени. Простыми словами, без юридических терминов.",
      },
    ],
  };
        ...
        <div className="grid grid-cols-1 items-center gap-14 lg:grid-cols-[1.1fr_1fr]">
          <div>
            ...
            <h1 className="mt-4 mb-4 text-[clamp(36px,5vw,56px)] font-extrabold leading-[1.08] tracking-[-0.02em]">
              Договор, страховка или счёт?{" "}
              <span className="hero-accent">Прочитайте мелкий шрифт.</span>
            </h1>
            <p className="mb-7 max-w-[540px] text-lg text-[var(--muted)]">
              Загрузите фото или PDF документа — найдём скрытые комиссии, автопродление и ловушки в мелком шрифте.
              Подскажем, когда ещё можно отказаться и вернуть деньги. Простыми словами, без юридических терминов.
            </p>
            ...
            <div className="mt-5 flex flex-wrap gap-5 text-[13px] text-[var(--muted)]">
              {["Фото или PDF", "Имя и паспорт скрыты от ИИ", "Понятный ответ за полминуты"].map((t) => (
          ...
          <div className="rounded-[20px] border border-[var(--card-border)] bg-[var(--card)] p-[22px] shadow-[0_30px_80px_-20px_rgba(0,0,0,0.6)]">
            <div className="mb-4 flex items-center gap-2.5">
              ...
              <span className="ml-2 text-[13px] text-[var(--muted)]">polis_strahovaniya_zhizni.pdf</span>
            </div>
            ...
            <p className="mb-3 text-[13px] italic leading-snug text-[var(--muted)]">
              Полис добровольного страхования жизни к ипотеке. Менеджер сказал «без него не одобрят», но по закону он добровольный. Действует 14 дней, чтобы отказаться и вернуть всю сумму.
            </p>
            {[
              ["Отправитель", "СК «Пример-Жизнь»"],
              ["Тип", "Полис страхования жизни (к ипотеке)"],
              ["Ключевые даты", "10.05.2026 — подписан\n24.05.2026 — последний день отказаться без потерь"],
              ["Суммы", "38 400 ₽ — взнос за первый год\n≈ 412 000 ₽ — за весь срок (10 лет)"],
            ].map(([k, v]) => (
            ...
            <div className="mt-4 flex items-start gap-2.5 border-l-2 border-[#f59e0b] pl-3">
              <span className="mt-1 h-2.5 w-2.5 shrink-0 rounded-full bg-[#f59e0b]" aria-hidden />
              <p className="m-0 text-[15px] font-semibold leading-snug">
                У тебя 14 дней, чтобы передумать. После 24 мая вернуть 412 000 ₽ за 10 лет уже не получится.
              </p>
            </div>
            ...
            <p className="mb-3 text-[13px] italic leading-snug text-[var(--text)]">
              Страховая хочет, чтобы ты платил взносы 10 лет. Банк — чтобы держал страховку и сохранял скидку к ставке по ипотеке. Решать тебе.
            </p>
          </div>
        </div>
      </div>
...
function FeaturesSection() {
  const items = [
    ...
    { ico: "💰", title: "Дешевле любой юр-системы", body: "Подписка на КонсультантПлюс или Гарант — это тысячи рублей в месяц и интерфейс для юристов. У нас один разбор стоит как чашка кофе, и ничего настраивать не надо." },
  ];
  return (
...
function HowSection() {
  const steps = [
    { title: "Загрузите", body: "Сфотографируйте письмо на телефон или прикрепите PDF из почты. Можно несколько страниц сразу." },
    { title: "Скрываем личное", body: "Имя, ИНН, паспорт и адрес заменяем на пометки вроде [имя], [инн]. В ИИ уходит обезличенный текст — ваши данные не утекают." },
    { title: "Читаем за вас", body: "Определяем, кто прислал, что хочет, какие суммы и сроки указаны и чем грозит просрочка." },
    { title: "Понятный ответ", body: "Что это, что делать сегодня и сколько у вас времени. Простыми словами, без юридических заклинаний." },
  ];
  return (
    <section id="how" className="py-20">
      <div className="mx-auto max-w-[1120px] px-6">
        ...
        <p className="mb-10 max-w-[680px] text-[17px] text-[var(--muted)]">
          Снаружи — простая форма: загрузил и получил ответ. Внутри — умный помощник, который читает официальные письма
          и переводит их с канцелярита на человеческий язык.
        </p>
...
function SafetySection() {
  const warns = [
    ...
    { title: "Загружайте только свои документы", body: "Сервис скрывает личные данные перед отправкой в ИИ, но это не разрешение работать с чужими бумагами. Не загружайте документы других людей без их согласия." },
    { title: "Договоры с банками и юр.лицами — нельзя", body: "В таких договорах почти всегда есть условие о коммерческой тайне или конфиденциальности. Их загрузка в любой внешний сервис (наш — не исключение) — это нарушение договора и риск личной ответственности. Разбирайте только свои личные документы." },
  ];
  return (
...
function FaqSection() {
  const items = [
    { q: "Мои персональные данные в безопасности?", a: "Перед отправкой в ИИ имя, ИНН, паспорт, СНИЛС и УИН мы заменяем на простые пометки — вроде [имя_1], [инн_1]. Нейросеть видит только обезличенный текст и ничего не запоминает." },
